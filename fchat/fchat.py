...
class OutgoingPumpThread(threading.Thread):
    def __init__(self, client):
        """
        This thread is used to send a steady "stream" of messages to the websocket. Since the F-Chat websocket does not
        like to be spammed with messages in rapid succession, we will used a buffer time to send them one at a time from
        a queue for safety.

        :param client: The websocket client object. Needed to check the outgoing buffer and to send messages. Usually
        called as self within the client.
        """
        super().__init__()
        self.name = "OutgoingPumpThread"
        self.client = client
        self.delay = 1
        self.running = True
    ...
    def run(self):
        """
        After the thread is started, use run() to start it up.

        To turn it off and end the thread, set running = False.
        :return:
        """
        while self.running:
    ...
    def set_delay(self, delay):
        """
        Used to set the delay between messages.
...
class PingTestThread(threading.Thread):
    def __init__(self, client):
        """
        The F-Chat websocket is constantly checking if we are still connected through "pings" in the form of PIN
        messages. It usually sends a ping every 30 seconds, and expects a ping back. If we do not send a ping in 90
        seconds, we are booted from the websocket. We will do something similar, where if we do not get a ping in 90
        seconds from F-Chat, we will assume we've lost connection and try to reconnect.

        :param client: The websocket client that should be closed after 90 seconds of inactivity. If called within the
        websocket client itself, simply define this as self.
        """
        threading.Thread.__init__(self)
        self.name = "ReconnectionThread"
        self.client = client
        self.running = True  # Turn this value to False to turn off the thread.
        self.tick = 0  # Number of seconds since last ping. Sets to 0 after every reset().

    def run(self):
        """
        After the thread is initialized, use run() to start the thread.

        To turn it off, set self.running = False.
        """
        while self.running:
            if self.tick == 90:
                self.client.logger.info("Didn't get a ping in time. Restarting.")
    ...
    def reset(self):
        """
        This function should be called whenever we get a ping from the server. This will reset our personal 90 second
        timer. If this timer reaches 90 seconds without a reset, our connection will close.
        """
        self.tick = 0
